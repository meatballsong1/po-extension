...
setInterval(function() {
    ...
    if (CONFIG.streamMaskEmail) {
        const alias = CONFIG.streamEmailAlias || 'diddy@rjktrades.com';
...
function injectStyles() {
    ...
    s.id = 'po-iife-styles';
    s.textContent = [
        // ------ NOTIFICATIONS (themed, with glow) ------
...
function buildPopoutWidget() {
    ...
    w.innerHTML =
        ...
        '<div id="po-float-bdy">' +
            ...
            field('pf-email-alias', 'Email Alias', 'diddy@rjktrades.com') +
    ...
    // Map of checkbox id -> storage key
    var BOOL_MAP = {
        'pf-master':       'isEnabled',
    ...
    // Close --- hides but keeps state
    document.getElementById('po-float-x').addEventListener('click', function() {
        w.style.display = 'none';
    });

    // Free drag --- no boundary clamping, full page freedom
...
function startEditor() {
    ...
    bar.innerHTML =
        ...
        '<span id="po-edit-hint">click text to edit  |  alt+drag to move</span>' +
    ...
    document.addEventListener('click',     edClick, true);
    document.addEventListener('mousedown', edDragStart, true);
...
function edStop() {
    ...
    document.removeEventListener('click',     edClick, true);
    document.removeEventListener('mousedown', edDragStart, true);
    document.querySelectorAll('.po-eh,.po-ea,.po-dh').forEach(function(el) {
        el.classList.remove('po-eh','po-ea','po-dh');
        el.removeAttribute('contenteditable');
    });
}

function inBar(el) {
    return !!(el && el.closest && (el.closest('#po-edit-bar') || el.closest('#po-float') || el.closest('#po-notif-root')));
}
function isEditable(el) {
    if (!el || !el.tagName) return false;
    var skip = ['SCRIPT','STYLE','HEAD','HTML','BODY','INPUT','TEXTAREA','IMG','SVG','PATH','CIRCLE','RECT'];
    if (skip.indexOf(el.tagName.toUpperCase()) !== -1) return false;
    return !!(el.innerText && el.innerText.trim().length > 0);
}

function edHover(e) {
    if (!editActive || inBar(e.target)) return;
    if (isEditable(e.target)) {
        e.target.classList.add(e.altKey ? 'po-dh' : 'po-eh');
    }
}
function edHout(e) {
    if (!editActive) return;
    e.target.classList.remove('po-eh','po-dh');
}

function edClick(e) {
    if (!editActive || inBar(e.target) || !isEditable(e.target)) return;
    e.preventDefault(); e.stopPropagation();
    document.querySelectorAll('.po-ea').forEach(function(a) {
        a.classList.remove('po-ea');
        a.removeAttribute('contenteditable');
        a.blur();
    });
    e.target.classList.add('po-ea');
    e.target.setAttribute('contenteditable', 'true');
    e.target.focus();
    e.target.addEventListener('blur', function onB() {
        e.target.removeAttribute('contenteditable');
        e.target.classList.remove('po-ea');
        var sel = getEl(e.target);
        if (sel) { editChanges[sel] = editChanges[sel] || {}; editChanges[sel].text = e.target.innerText; }
        e.target.removeEventListener('blur', onB);
    }, { once: true });
}

var dragEl = null, dox = 0, doy = 0;
function edDragStart(e) {
    if (!editActive || !e.altKey || inBar(e.target) || !isEditable(e.target)) return;
    e.preventDefault(); e.stopPropagation();
    dragEl = e.target;
    var r = dragEl.getBoundingClientRect();
    dox = e.clientX - r.left; doy = e.clientY - r.top;
    dragEl.style.position = 'fixed';
    dragEl.style.zIndex   = '2147483640';
...
function edDragEnd() {
    if (!dragEl) return;
    var sel = getEl(dragEl);
    if (sel) {
        editChanges[sel] = editChanges[sel] || {};
        editChanges[sel].x = dragEl.style.left;
        editChanges[sel].y = dragEl.style.top;
        editChanges[sel].fixed = true;
    }
    dragEl = null;
    document.removeEventListener('mousemove', edDragMove);
    document.removeEventListener('mouseup',   edDragEnd);
}
...
function getEl(el) {
    try {
        ...
        var p = [], c = el;
        for (var i = 0; i < 4 && c && c !== document.body; i++) {
    ...
    try {
        ...
        if (Object.keys(ch).length > 0) {
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', function() { setTimeout(function() { applyLayout(ch); }, 600); });
            } else {
                setTimeout(function() { applyLayout(ch); }, 600);
            }
        }
    } catch(e) {}
});
...
// ============================================================
var CHANGELOG = {
    version: '2.3',              // Change this string to trigger the popup again for all users

    title: 'welcome to version 2.3!',   // Big heading
    subtitle: 'this is a PRETTY big update', // Small subheading under title

    // OPTIONAL: path to an image file in your extension folder.
    // Set to '' or null to show no image.
    image: 'https://cdn.discordapp.com/attachments/1478916523820060834/1484004106786967572/image.png?ex=69bca606&is=69bb5486&hm=9910636aad93ba2a08f429dbee3814d84dfa03d74b6bff220fe9f1c28a9ed823&',                   // e.g. 'changelog-banner.png'

    // CONTENT MODE - pick ONE:
    //   'bullets'  - shows a bulleted list (default)
    //   'text'     - shows a plain paragraph, no bullets
    //   'none'     - no body content at all, just title + button
    mode: 'bullets',

    // Used when mode is 'bullets' - each string becomes one bullet
    items: [
    ...
    // Used when mode is 'text' - one block of plain text
    text: '',

    // Used when mode is 'links' - each item is { text: 'label', url: 'https://...' }
    // Items can also just be strings (renders as plain unlinked text in that row)
    // Example:
    //   items: [
...
function showChangelog() {
    ...
    if (CHANGELOG.mode === 'bullets' && CHANGELOG.items && CHANGELOG.items.length) {
        ...
        var rows = CHANGELOG.items.map(function(item) {
            if (typeof item === 'string') {
                return '<li class="po-cl-link-row"><span class="po-cl-link-plain">' + item + '</span></li>';
            }
            return '<li class="po-cl-link-row">' +
                '<a class="po-cl-link" href="' + item.url + '" target="_blank" rel="noopener">' +
                    '<span class="po-cl-link-text">' + item.text + '</span>' +
    ...
    // mode === 'none' leaves bodyHTML empty

    // Build optional image
    // image can be:
    //   a URL  -> 'https://i.imgur.com/abc.png'   (rendered directly)
    //   a file -> 'banner.png'                     (loaded from extension folder)
    //   ''     -> no image shown
    var imgHTML = '';
    if (CHANGELOG.image) {
...
// showPageNotif / buildPopoutWidget / startEditor defined in IIFE below
